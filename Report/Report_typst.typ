...
== Integrity and non-malleability

The service protects message integrity and non-malleability at two levels.

First, the secure channel uses *ChaCha20Poly1305*, an authenticated encryption scheme. Each encrypted JSON message includes a Poly1305 authentication tag. The receiver verifies this tag before accepting the plaintext. Therefore, any modification to the ciphertext, nonce, or encrypted fields causes decryption to fail, preventing attackers from altering requests, credentials, hashes, balances, or timestamp responses.

Second, timestamp tokens are protected with *RSA-PSS* signatures. The TSA signs the exact bundle $"hash" || "timestamp"_"bytes"$ using $"privK"_"ts"$ and *SHA256*. During verification, the same bundle is reconstructed and checked with $"pubK"_"ts"$. If the hash, timestamp, or signature is modified, verification fails.

Thus, ChaCha20Poly1305 provides integrity and non-malleability for the communication channel, while RSA-PSS provides integrity and non-malleability for timestamp tokens after they leave the channel.
...
The public key $"pubK"_"c"$ is bound to the server certificate _certKc.pem_. The client already has the authentic certificate before the connection starts and uses it as a local trust anchor. It verifies the certificate, extracts $"pubK"_"c"$ from it, and then uses that key to verify the server's handshake signature.
...
1. The client loads the trusted server certificate _certKc.pem_ and extracts the certified connection-authentication public key.
...
Each nonce is valid only once. A captured old request contains an old nonce, so it will not match the server's current expected value.
...
1. *Trusted server certificate distribution*
  ...
  The client already has the authentic _certKc.pem_ file. This certificate contains $"pubK"_"c"$ and is used as the local trust anchor for server authentication. In a real deployment, this certificate would need to be distributed through a trusted installation process or replaced by a certificate chain anchored in a trusted CA.
...
The project implements a simple timestamping service: it establishes an authenticated encrypted channel, protects user credentials, prevents simple replay of session commands, signs timestamp tokens, and verifies them later.

The most important design idea is separation of responsibilities. X25519 and HKDF create fresh session keys, ChaCha20Poly1305 protects the communication, the server certificate binds the connection-authentication public key to the TSA, RSA-PSS signs timestamp tokens, bcrypt protects stored passwords, and rotating replay nonces make captured requests unusable after they have been processed once.

The service lets a user prove that a document hash existed at a specific UTC time, while also showing how several cryptographic primitives cooperate inside a realistic client-server protocol.
